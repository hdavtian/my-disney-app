...
public class MovieController {
    ...
    @Operation(summary = "Batch fetch movies by IDs", description = "Retrieves multiple movies in a single request by providing a comma-separated list of movie IDs. "
            +
            "Optimized for loading favorited items without fetching entire collections.", tags = { "Movies" })
    @ApiResponses(value = {
    ...
    public ResponseEntity<List<Movie>> getMoviesByIds(
            @Parameter(description = "Comma-separated list of movie IDs", example = "1,5,12,23,45", required = true) @RequestParam List<Long> ids) {
        List<Movie> movies = movieService.findByIds(ids);
        return ResponseEntity.ok(movies);
    }
}